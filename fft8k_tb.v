...
module fft8k_tb;
   ...
   reg reset_n;
   reg sink_valid;
   reg[32 - 1:0] sink_real;
   reg[32 - 1:0] sink_imag;
   wire [13:0]    fftpts_in;
   ...
   wire [32 - 1: 0] source_real;
   wire [32 - 1: 0] source_imag;
   parameter 		       NUM_FRAMES_c = 4;
   parameter 		       MAXVAL_c = 2**(32 -1);
   parameter 		       OFFSET_c = 2**(32);
   ...
   reg [32 - 1: 0] data_real_in_hex;
   reg [32 - 1: 0] data_imag_in_hex;
   reg [32 - 1: 0] fft_real_out_hex;
   reg [32 - 1: 0] fft_imag_out_hex;
...
initial
   begin
     ...
     fftpts_array[2]=8192;
     fftpts_array[3]=16;
     data_rf = $fopen("fft8k_real_input.txt","r");
     data_if = $fopen("fft8k_imag_input.txt","r");
     fft_rf = $fopen("fft8k_real_output_ver.txt");
     fft_if =$fopen("fft8k_imag_output_ver.txt");
     fft_blksizef = $fopen("fft8k_blksize_report.txt");
     fft_inversef = $fopen("fft8k_inverse_report.txt");
     #0 clk = 1'b0;
     #0 reset_n = 1'b0;
     #92 reset_n = 1'b1;
  end
  ...
  always @(posedge clk)
   ...
   always @ (posedge clk)
	...
	if(reset_n==1'b0)
	  ...
	  begin
		 ...
		 begin
                    rc_x = $fscanf(data_rf,"%x",data_real_in_hex);
                    sink_real <= data_real_in_hex;
                    ic_x = $fscanf(data_if,"%x",data_imag_in_hex); 
		    sink_imag <= data_imag_in_hex;
		    sink_valid <= 1'b1;
		 end 
               else
		 begin
   ...
   always @ (posedge clk) 
	...
	if((reset_n==1'b1) & (source_valid == 1'b1 & source_ready == 1'b1))
	  begin
	     $fdisplay(fft_rf, "%X", source_real);
             $fdisplay(fft_if, "%X", source_imag);
	  end
	end
