...
set quartus_rootdir C:/altera/11.0sp1/quartus/
...
if {[string match $flow "rtl"] } {
    ...
    set quartus_libs [list \
        ...
        wysiwyg     {stratix_atoms stratix_components} {stratix}   "$q_sim_lib" ]
    ...
    if {[file exists fir.vhd]} {vcom -93 -work work fir.vhd}
    if {[file exists fir.v]} {vlog -work work fir.v}
    vcom -93 -work work fir_ast.vhd
    vcom -93 -work work fir_rtl.vhd

    if { [file exists fir_rtl_wysiwyg.vhd] } {
        vcom -93 -work work fir_rtl_wysiwyg.vhd
    }
