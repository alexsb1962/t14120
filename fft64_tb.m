...
% file : fft64_tb.m
%
% Description : The following Matlab testbench excercises the Altera FFT Model fft64_model.m
...
fidnps = fopen('fft64_blksize_report.txt','r');
fidinv = fopen('fft64_inverse_report.txt','r');
% Note: fft64_blksize_report.txt is generated when the HDL simulation is run, so if it does
% not exist then flag an error
if fidnps == -1 
  msgbox('Error: fft64_blksize_report does not exist, run the HDL simulation first.', 'fft64_blksize_report.txt missing', 'error');
elseif fidinv == -1 
  msgbox('Error: fft64_inverse_report does not exist, run the HDL simulation first.', 'fft64_inverse_report.txt missing', 'error');
else
  % Read input complex vector, and transform sizes from source text files 
  fidr = fopen('fft64_real_input.txt','r');                                            
  fidi = fopen('fft64_imag_input.txt','r');
  ...
  x = xreali' + j*ximagi';
  [y] = fft64_model(x,nps,inverse); 
  fidro = fopen('fft64_real_output_c_model.txt','w');                                  
  fidio = fopen('fft64_imag_output_c_model.txt','w');
  realout = real(y);
  imagout = imag(y);
